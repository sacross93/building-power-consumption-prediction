# -*- coding: utf-8 -*-
"""
Energy Consumption Forecast v7 - Optuna 자동 튜닝
----------------------------------------------------
* 핵심 개선사항:
  1. Optuna를 이용한 자동 하이퍼파라미터 최적화
  2. 고급 시계열 피처 (푸리에 변환, 계절성 분해)
  3. 건물별 개별 모델링
  4. 다중 앙상블 (LightGBM + XGBoost)
  5. 시계열 특화 검증
  6. 후처리 및 앙상블
"""
import argparse
...
def objective(trial, X_train, y_train, X_val, y_val, cat_cols, model_type='lightgbm'):
    """Optuna 목적 함수"""
...
def optimize_hyperparameters(X_train, y_train, X_val, y_val, cat_cols, model_type='lightgbm', n_trials=50):
    """하이퍼파라미터 최적화"""
    study = optuna.create_study(direction='minimize')
    
    objective_with_params = lambda trial: objective(trial, X_train, y_train, X_val, y_val, cat_cols, model_type)
    study.optimize(objective_with_params, n_trials=n_trials)
    
    print(f"Best {model_type} SMAPE: {study.best_value:.4f}%")
...
def train_lightgbm_optimized(X_train, y_train, X_val, y_val, cat_cols, n_trials=50):
    """Optuna로 최적화된 LightGBM 모델 학습"""
    print("LightGBM 하이퍼파라미터 최적화 중...")
    best_params = optimize_hyperparameters(X_train, y_train, X_val, y_val, cat_cols, 'lightgbm', n_trials)
...
def train_xgboost_optimized(X_train, y_train, X_val, y_val, n_trials=50):
    """Optuna로 최적화된 XGBoost 모델 학습"""
    print("XGBoost 하이퍼파라미터 최적화 중...")
    best_params = optimize_hyperparameters(X_train, y_train, X_val, y_val, None, 'xgboost', n_trials)
...
def train_per_building_optimized(df_train, df_test, features, cat_cols, n_trials=50):
    """건물별 개별 모델 학습 (Optuna 최적화)"""
    ...
    for building_id in df_train["건물번호"].unique():
        ...
        # 최종 모델들 (전체 데이터로 최적화)
        print("  최종 모델 학습 중...")
        final_model_lgb = train_lightgbm_optimized(X_final_scaled, y, X_final_scaled, y, cat_cols, n_trials//2)
        final_model_xgb = train_xgboost_optimized(X_final_scaled, y, X_final_scaled, y, n_trials//2)
...
def run_pipeline(train_path, test_path, info_path, n_trials=50):
    """메인 파이프라인"""
    print("데이터 로딩 중...")
    ...
    sub_df = train_per_building_optimized(df_train, df_test, feats, ["건물번호"], n_trials)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--n-trials", type=int, default=50, help="Optuna 최적화 시도 횟수")
    args = ap.parse_args()
